...
def write_text_macros(frame: pd.DataFrame, output_dir: str | Path) -> Path:
    ...
    # Positive-tickpath count at the baseline cost, for the pooled unfairness rate.
    poscount = pd.to_numeric(frame["num_amu_tickpath"], errors="coerce") if "num_amu_tickpath" in frame.columns else pd.Series(np.nan, index=frame.index)

    def wmean(series: pd.Series, mask: pd.Series) -> float:
        a, w = series[mask], weights[mask]
        ok = a.notna() & w.notna() & (w > 0)
        return float(np.average(a[ok].to_numpy(float), weights=w[ok].to_numpy(float))) if ok.any() else float("nan")

    def pooled_rate(mask: pd.Series) -> float:
        # Unfairness rate R pooled exactly as the cost-sensitivity table:
        # sum(positive tickpaths) / (4 * sum(n_obs)). Cells with zero positive
        # paths still contribute their n_obs to the denominator, so this reproduces
        # write_cost_sensitivity_table_from_panel's baseline column to the digit
        # (a cell-day mean of the ratio uncond/cond would instead drop those cells
        # -- their ratio is NaN -- and bias R upward).
        pc, w = poscount[mask], weights[mask]
        ok = pc.notna() & w.notna() & (w > 0)
        denom = PATHS_PER_TICK * float(w[ok].sum())
        return float(pc[ok].sum() / denom) if denom > 0 else float("nan")

    rate_mean, rate_pre, rate_post = pooled_rate(poscount.notna()), pooled_rate(post == 0.0), pooled_rate(post == 1.0)
    macros = {
        "CostBp": f"{cost_bp}",
        "AmuMeanBp": f"{wmean(amu, amu.notna()):.2f}",
        "AmuPreBp": f"{wmean(amu, post == 0.0):.2f}",
        "AmuPostBp": f"{wmean(amu, post == 1.0):.2f}",
        # Unfairness rate R (fraction), pooled over tickpaths at the baseline cost.
        "AmuMeanRate": f"{rate_mean:.3f}",
        "AmuPreRate": f"{rate_pre:.3f}",
        "AmuPostRate": f"{rate_post:.3f}",
        "AmuDeltaRate": f"{rate_post - rate_pre:+.3f}",
    }
...
def write_cost_sensitivity_table_from_panel(panel: pd.DataFrame, output_dir: str | Path) -> Path:
    ...
    lines = [r"\begin{tabular}{rrrr}", r"\toprule", r"cost (bp) & $R$ pre & $R$ post & $\Delta R$ \\", r"\midrule"]
    for cost_bp in _COST_TABLE_BP:
        pre_v = _panel_pooled_amu(pre, cost_bp, conditional=False, metric="rate")
        post_v = _panel_pooled_amu(post, cost_bp, conditional=False, metric="rate")
        lines.append(rf"{cost_bp} & {pre_v:.3f} & {post_v:.3f} & {post_v - pre_v:+.3f} \\")
...
# "Drop stale quotes" on the aggregated panel: drop the stalest decile of cell-days.
# (The tick-level version excluded individual stale ticks, which the pre-aggregated
# panel cannot reproduce; dropping the highest-stale-fraction cell-days is the panel
# analogue, and removes a meaningful 10% rather than only the fully-stale cells.)
_ROBUSTNESS_STALE_DROP_Q = 0.90
_ROBUSTNESS_CLIP_CAP_BP = int(round(float(CONFIG["filters"]["max_amu_bp"])))


def _pooled_rate_panel(sub: pd.DataFrame, *, weight_col: str | None = None) -> float:
    """Unfairness rate R pooled over a panel subset: sum(positive tickpaths) /
    (4 * sum(n_obs)). When ``weight_col`` is given, each cell-day is weighted by that
    column in both numerator and denominator (the notional-weighted variant)."""
    pc = pd.to_numeric(sub["num_amu_tickpath"], errors="coerce")
    n = pd.to_numeric(sub["n_obs"], errors="coerce")
    w = pd.to_numeric(sub[weight_col], errors="coerce").fillna(0.0) if weight_col else pd.Series(1.0, index=sub.index)
    ok = pc.notna() & n.notna()
    denom = PATHS_PER_TICK * float((w[ok] * n[ok]).sum())
    return float((w[ok] * pc[ok]).sum() / denom) if denom > 0 else float("nan")


def write_robustness_grid_table_from_panel(panel: pd.DataFrame, output_dir: str | Path) -> Path:
    """Unfairness rate R pre/post-2024 under a grid of filter/aggregation choices,
    computed on the SAME filtered cell-day panel the regressions and the
    cost-sensitivity table use -- so the baseline row reproduces the cost table's
    baseline-cost column to the digit. (Previously computed on the raw tick stream,
    which pooled a slightly different sample and left the baseline ~0.001 off.)

    R is independent of the positive-part clip cap (it counts positive tickpaths,
    not their magnitude), so the three clip rows equal the baseline by construction;
    they are retained to document that invariance. ("Widen to all strikes" is omitted:
    the panel is aggregated within the baseline moneyness band, so there are no wider
    strikes to add back; the moneyness check is the one-sided narrowing row.)
    Output: robustness_grid_table.tex."""
    cap = _ROBUSTNESS_CLIP_CAP_BP
    # (label, filter_analysis_panel overrides, drop-stale?, weight column)
    settings: list[tuple[str, dict, bool, str | None]] = [
        ("baseline", {}, False, None),
        (rf"clip {cap // 2}~bp", {}, False, None),
        (rf"clip {cap * 2}~bp", {}, False, None),
        ("no clip", {}, False, None),
        ("drop stale quotes", {}, True, None),
        ("spread cap 500~bp", dict(spread_bp_max=500), False, None),
        ("spread cap 200~bp", dict(spread_bp_max=200), False, None),
        ("min size \\$25k", dict(min_quote_size_dollar=25_000.0), False, None),
        ("moneyness $\\pm10\\%$", dict(rel_strike_min=0.9, rel_strike_max=1.1), False, None),
        ("notional-weighted", {}, False, "mean_min_quote_size_dollar"),
    ]
    lines = [r"\begin{tabular}{lrrr}", r"\toprule", r"setting & $R$ pre & $R$ post & $\Delta R$ \\", r"\midrule"]
    for i, (label, overrides, drop_stale, weight_col) in enumerate(settings):
        sub = filter_analysis_panel(panel, **overrides)
        if drop_stale and "stale_proxy" in sub.columns:
            st = pd.to_numeric(sub["stale_proxy"], errors="coerce")
            sub = sub[st <= st.quantile(_ROBUSTNESS_STALE_DROP_Q)]
        pre = sub[sub["post_2024"] == 0.0]
        post = sub[sub["post_2024"] == 1.0]
        pre_v = _pooled_rate_panel(pre, weight_col=weight_col)
        post_v = _pooled_rate_panel(post, weight_col=weight_col)
        lines.append(rf"{label} & {pre_v:.3f} & {post_v:.3f} & {post_v - pre_v:+.3f} \\")
        if i == 0:
            lines.append(r"\midrule")
    lines += [r"\bottomrule", r"\end{tabular}"]
    path = Path(output_dir) / "robustness_grid_table.tex"
    path.write_text("\n".join(lines) + "\n")
    logger.debug("write_robustness_grid_table_from_panel wrote %s", path)
    return path
...
def write_regression_tables(output_dir: str | Path, *, frame: pd.DataFrame | None = None, **build_kwargs):
    ...
    _reg_tags = ("tab:regression_coefficients", "regression_coefficients_table",
                 ...
                 "tab:robustness_grid", "robustness_grid_table",
    ...
    paths["robustness_grid_table"] = write_robustness_grid_table_from_panel(panel, output_root)
    logger.debug(
