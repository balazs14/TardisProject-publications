...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python publications/enable_draft_mode.py path/to/file.tex")
        return 1
    ...
    content = content.replace(NORMAL_HEADER, DRAFT_HEADER, 1)
    content = content.replace(NORMAL_BIB_FOOTER, DRAFT_BIB_FOOTER, 1)

    if content != original:
        tex_path.write_text(content, encoding="utf-8")
        print(f"Enabled draft mode in {tex_path}")
    else:
