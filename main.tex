...
The data used in this paper is sourced from Tardis, a comprehensive provider of high-frequency financial data. Tardis offers granular market data across a wide range of cryptocurrency exchanges, including but not limited to Binance, Deribit, and OKX. The dataset encompasses various types of derivative products such as standard and inverse futures and options. We are able to arrange these data to study mathematically rigorous put-call parity relationships. Approximate PCP relationships also exist which involve perpetual futures and spot trading, but in this paper we focus on expiration futures and options.

The current paper is based on the OKEX data from Tardis, for approximately 1 year from the beginning of 2025 to the end of february 2026. The table \ref{fig:summary_df} is a summary of how many put-call pairs we have been able to study.


\begin{figure}[!htbp]
\centering
\includegraphics[width=0.9\textwidth]{summary_df_display.png}
\caption{Summary statistics of OKX put-call pairs: number of contracts, daily average volume, and liquidity measures.}
...
%Considering trading fees, borrowing, and lending rates, we need further modifications on the PCP constraints. Only for the inverse future options, the full expression for the forward arbitrage resuis:
...
Not only the type of underlying, but also the modified pay-off structure compared to the standard option contract requires adjustments in PCP equations. As \cite{alexander2023crypto} already published a detailed overview of inverse, standard and inverse quanto options including the pay-off functions and the formulae for pay-offs, we immediately present the adjusted PCP parities based on their results in XXXX?? %HOGY A FENÉBE KELL EZT AZ ALÁBBI VACKOT BEHIVATKOZNI? SORRY, NEM SIKERÜL.(Nóri)
...
%ITT HAGYTAM EGYELŐRE ABBA. KATA, JÓ LESZ ÍGY? HA IGEN, MEGYEK TOVÁBB EBBEN A STÍLUSBAN. (Nóri)
In contrast, a standard quanto option

has:
\begin{itemize}
    \item underlying denominated in a \emph{foreign} currency,
    \item payoff delivered in a \emph{domestic} currency,
    \item FX risk is removed via a quanto structure.
\end{itemize}

The key consequence: the drift of the foreign-denominated asset under the
domestic measure contains the \emph{quanto adjustment}:
\[
q^{Q} = q + \rho \sigma_S \sigma_{FX}.
\]

This modification enters the domestic forward price of the underlying.

\subsection*{Parity}
\begin{equation}
C^{Q} - P^{Q}
= S_0 \, e^{-q^{Q}T} - K e^{-r_d T},
\qquad q^{Q} = q + \rho \sigma_S \sigma_{FX}.
\end{equation}

\subsection*{Interpretation}
The structure resembles the vanilla parity, but the quanto-adjusted dividend
rate $q^{Q}$ replaces the standard yield $q$. The strike is discounted at the
\emph{domestic} risk-free rate $r_d$.

The quanto adjustment reflects the correlation between the underlying
and the FX rate. A positive correlation increases the effective dividend
yield and lowers the domestic forward price; a negative correlation does the opposite.
...
X_t = \frac{1}{S_t}.
\]
The payoff is in domestic currency and FX risk is neutralized (quanto).
Thus, a quanto adjustment still appears, but because the underlying is
a reciprocal, the sign changes relative to the standard quanto case.
...
\subsection*{Interpretation}
This is a direct analogue of the standard quanto parity, but with
$1/S_0$ as the effective underlying. Because $1/S$ has opposite
sensitivity to $S$, the quanto effect changes accordingly.

\section{Results}

\subsection*{AMU arbitrage paths}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{okex_4_spreads.png}
\caption{Distribution of the 4 versions of the AMU arbitrage on OKX. Both BTC and ETH are shown. Note that only the positive tail corresponds to actual tradable opportunities}
\label{okex_4_spreads}
\end{figure}

In figure \ref{okex_4_spreads} we show the distribution of the 4 versions of the AMU arbitrage on OKX. Only the positive tail corresponds to actual tradable opportunities, while the negative tail corresponds to non-tradable situations. The peak around -30 bp shows that most of the time, quoting is NOT profitable as a path to risk-free profit via the PCP breaking relationship.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{okex_amu_bps_by_date.png}
\caption{Time evolution of AMU (in basis points) on OKX for BTC and ETH options}
\label{okex_amu_by_date}
\end{figure}

In figure \ref{okex_amu_by_date} we show the time evolution of the AMU in OKEX. When superimposed with the BTCUSD spot value, it is notable how the AMU becomes much higher during times of market stress, namely times of sudden BTC price drops.

The bottom pane of figure \ref{okex_amu_by_date} shows how often the AMU arbitrage actually sets up out of all times when we have valid put and call markets. The notable weekly structure of the number of ticks has to do with a data artifact pertaining to Fridays in the raw Tardis data.

\subsection*{AMU by Time-to-Expiration}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{okex_amu_bps_by_tte.png}
\caption{AMU (in basis points) on OKX stratified by time-to-expiration (TTE) for BTC and ETH options}
\label{okex_amu_by_tte}
\end{figure}

TTE refers to the time to expiration of the option pairs observed, measured in years. As shown in figure \ref{okex_amu_by_tte}, the AMU is worse for farther dated options. However, there are very few observations available for longer dated contracts, most of the data is for options expiring in two weeks or less.

\subsection*{AMU by Relative Strike}

Relative strike is the conventional measure $K/F$, where $K$ is the strike price and $F$ is the forward price of the underlying. Relative strike = 1 corresponds to a put-call pair where both contracts are at the money. For relative strike < 1, the call is in the money and the put is out of the money, while for relative strike > 1, the put is in the money and the call is out of the money. As shown in figure \ref{okex_amu_by_strike}, the AMU is worse for out-of-the-money options, and is best for at-the-money options. This is consistent with the fact that liquidity is usually the best for at-the-money options, and the worst for out-of-the-money options, which makes it more likely to observe PCP breaking.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\textwidth]{okex_amu_bps_by_rel_strike.png}
\caption{AMU (in basis points) on OKX stratified by relative strike (moneyness) for BTC and ETH options}
\label{okex_amu_by_strike}
\end{figure}
...
The differences between the four versions arise entirely from what the
``effective underlying'' is and whether a quanto adjustment modifies its
domestic-currency forward price.


\section*{Final Remarks}

\begin{itemize}
    \item Put--call parity does not depend on the drift of the underlying under
    the real-world measure. It is purely a no-arbitrage statement.
    \item Quanto structures modify the domestic forward price because the change
    of measure introduces correlation-dependent drift corrections.
...
&\quad - U(4\tau^{opt} + 2\tau^{und}) && \text{correct for trading cost}\\
&\quad + \max(\delta_U, \delta_C, \delta_P) && \text{gain the biggest spread}
\end{aligned}
\end{equation}
...
crypto       &  0.53 & 1237 bp  &  650 bp\\
spx          &  0.25 &   61 bp  &   15 bp\\
    \end{tabular}
...
\section{Conclusions}
The final results of our investigation is in Table \ref{tab:AMU}.
